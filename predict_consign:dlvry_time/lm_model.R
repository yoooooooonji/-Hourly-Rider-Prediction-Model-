...
results_dlvry <- data.frame(rgn1_nm = character(), r2 = numeric(), coef = numeric(),  range = numeric(), stringsAsFactors = FALSE)
results_consign <- data.frame(rgn1_nm = character(), r2 = numeric(), coef = numeric(), range = numeric(),  stringsAsFactors = FALSE)
results_over_20 <- data.frame(rgn1_nm = character(), r2 = numeric(), coef = numeric(), range = numeric(), stringsAsFactors = FALSE)
...
get_dlvry_lm <- function(data){
       model_dlvry <- lm(avg_dlvry_diff ~ rider_cnt + ord_cnt + weekday + mean_fee, data = data)
        ...
        #  predictions <- predict(model_dlvry, interval = "predict", level = 0.95)
        # lower_bound <- predictions - 1.96 * summary(model_dlvry)$sigma
        # upper_bound <- predictions + 1.96 * summary(model_dlvry)$sigma
...
get_consign_lm <- function(data){
        ...
        # predictions <- predict(model_consign, interval = "predict", level = 0.95)
        # lower_bound <- predictions - 1.96 * summary(model_consign)$sigma
        # upper_bound <- predictions + 1.96 * summary(model_consign)$sigma
...
get_over_20_lm <- function(data){
       model_over20 <- lm(over_20 ~ rider_cnt + ord_cnt + weekday + mean_fee, data = data)
        ...
        # predictions <- predict(model_over20, interval = "predict", level = 0.95)
        # lower_bound <- predictions - 1.96 * summary(model_over20)$sigma
        # upper_bound <- predictions + 1.96 * summary(model_over20)$sigma
        range <- 1.96 * summary(model_over20)$sigma
        result <- data.frame(r2, coef, range)
        result
}
...
# y_pred <- predict(model_dlvry, newdata = se,interval = "predict", level = 0.95) 
# y_pred[, "lwr"]
# library(Metrics)
