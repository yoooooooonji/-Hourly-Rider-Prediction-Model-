...
##########################################################################################################################################################
# data load
data <- read_excel("/Users/yj.noh/Desktop/train_data_daegu.xlsx")
...
# seoul
data <- data %>% 
  filter(pick_rgn2_nm == '중구' & reg_date < max(data$reg_date))
...
datetime_full <- seq(as.POSIXct("2023-01-01 09:00"), as.POSIXct("2023-06-05 23:00"), by = "1 hour")
...
weather <- read.csv("/Users/yj.noh/Desktop/weather_daegu.csv", fileEncoding = "cp949")
...
table(combined_data$is_rain) # 0: 2223, 1 : 111

combined_data <- combined_data %>% 
group_by(reg_date) %>% 
mutate(is_rain2 = ifelse(sum(is_rain) >0 ,1,0))

table(combined_data$is_rain2) # 2035, 299
...
table(combined_data$holiday_yn) #1616, 718
...
table(train_data$outlier) # 1769 ,25
...
write.csv(combined_data, "hour_daegu/combined_data.csv", row.names = FALSE, fileEncoding = "cp949")
