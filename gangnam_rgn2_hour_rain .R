...
train_set <- subset(train, select = -c(datetime, reg_date, order_cnt, rider_cnt, order_cnt_2, is_rain, rain_c, snow_c, pick_rgn2_nm))
test_set <- subset(test, select = -c(datetime, reg_date,  order_cnt, rider_cnt, order_cnt_2, is_rain, rain_c, snow_c, pick_rgn2_nm))
...
# 1~2
model5 <- lm(formula = rider_cnt_2 ~ day_of_reg + temp_c + 
               month + week + order_cnt_w_1 + 
...
# 1~3
model6 <- lm(formula = rider_cnt_2 ~  day_of_reg + temp_c + 
              month + week + order_cnt_w_1 + 
...
model_AIC <- lm(formula = rider_cnt_2 ~ hour_reg + day_of_reg + temp_c + month + 
                  week + order_cnt_w_1 + order_cnt_w_2 + order_cnt_w_4 + rider_cnt_w_1 + 
                  rider_cnt_w_2 + rider_cnt_w_3 + rider_cnt_w_4 + is_holiday, 
                data = train_set)
...
# p-value < 0.05 
summary(model)
model_fin <- lm(rider_cnt_2 ~ hour_reg + day_of_reg + temp_c + month + week + order_cnt_w_1 + order_cnt_w_2 + order_cnt_w_3 + 
                  order_cnt_w_4 + rider_cnt_w_1 + rider_cnt_w_2 + rider_cnt_w_3 + 
...
y_pred <- predict(model_fin, newdata = test_set) # 모델의 예측값
